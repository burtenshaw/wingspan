...
now = datetime.now()
yesterday = pd.to_datetime('2021-01-25 12:25:47.657977')

mdf = pd.DataFrame()

for method in ['ALBERT', 'ELECTRA', 'ROBERTA']:
    bdf = pd.concat([get_best(method, n, since = yesterday, n=1) for n in list(range(5)) + ['all']], axis = 0)
    bdf['method'] = method
...
BEST = mdf.to_dict(orient='index')

# output_path = 'data/best_hparams.json'
# with open(output_path, 'w') as f:
#     json.dump(output, f)

# mdf.drop(columns=['fold']).reset_index().to_json('data/best_hparams.json')
# %%
