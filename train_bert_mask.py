...
data = make_word_level_df(data, max_len = MAX_LEN)

#%%
X_train_id, _, X_train_attn = bert_prep(data.loc[train_index].text.to_list(), max_len = MAX_LEN)
X_val_id, _, X_val_attn = bert_prep(data.loc[val_index].text.to_list(), max_len = MAX_LEN)
X_test_id, _, X_test_attn = bert_prep(data.loc[test_index].text.to_list(), max_len = MAX_LEN)
...
while now < tomorrow:
    ...
    with tf.summary.create_file_writer(run_dir).as_default():
        ...
        results = ngram_single_bert(data = train_samples,
