...
train['parsing_predictions'] = train.apply(word_mask_to_character_entity, pad = False, axis = 1)
print(' parsing accuracy %s' % train.apply(lambda row : \
                            f1(row.parsing_predictions, row.entities), axis = 1).mean())
...
train['tuples'] = train[['word_mask', 'tokens']].apply(lambda row : list(enumerate(zip(row.tokens, row.word_mask))), axis=1)
